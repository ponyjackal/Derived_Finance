...
async function main() {
  ...
  const synthetix = new ethers.Contract('0xc6e7DF5E7b4f2A278906862b61205850344D4e7d', SynthetixABI, owner);
  ...
  await synthetix["transfer(address,uint256)"](addr1.address, ethers.utils.parseEther("1000000"));
  // await synthetix.transfer(addr1.address, ethers.utils.parseEther("1000000"));

  console.log("address 1 Synthetix balance : ", await synthetix.balanceOf(addr1.address));
  
  // send some DVDX to depot contract
  await synthetix["transfer(address,uint256)"]('0xcbEAF3BDe82155F56486Fb5a1072cb8baAf547cc', ethers.utils.parseEther("20000000"));
  console.log("Depot contract Synthetix balance : ", await synthetix.balanceOf('0xcbEAF3BDe82155F56486Fb5a1072cb8baAf547cc'));
  // issue synth in address1
  const usdx = new ethers.Contract('0xf5059a5D33d5853360D16C683c16e67980206f36', SynthABI, addr1);
  // const USDx = await hre.ethers.getContractFactory("Synth");
  // const usdx = await USDx.attach('0xf5059a5D33d5853360D16C683c16e67980206f36');

  await synthetix.connect(addr1).issueSynths(ethers.utils.arrayify('0x55534478'), ethers.utils.parseEther("10000"));

  console.log("address 1 USDx balance : ", await usdx.balanceOf(addr1.address));

  // send some USDx to address2
  await usdx["transfer(address,uint256)"](addr2.address, ethers.utils.parseEther("10000"))
  console.log("address 1 USDx balance : ", await usdx.balanceOf(addr1.address));
  console.log("address 2 USDx balance : ", await usdx.balanceOf(addr2.address));

  // try to exchange USDx for DVDx
  const Depot = await hre.ethers.getContractFactory("Depot",{
    libraries: {
      SafeDecimalMath: '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
    },
  });
  const depot = await Depot.attach('0xcbEAF3BDe82155F56486Fb5a1072cb8baAf547cc');
  // approve USDx to Depot contract
  await usdx.connect(addr2).approve(depot.address, ethers.utils.parseEther("1000"));
