...
describe("Depot", async (accounts) => {
  ...
  const sUsdHex = w3utils.asciiToHex("sUSD");
  ...
  beforeEach(async () => {
    ...
    Synthetix = await ethers.getContractFactory("Synthetix", {
      libraries: {
        SafeDecimalMath: safeDecimalMath.address,
      },
    });
    Depot = await ethers.getContractFactory("Depot", {
      libraries: {
        SafeDecimalMath: safeDecimalMath.address,
      },
    });
  ...
  it("should set constructor params on deployment", async () => {
    ...
    const instance = await new Depot(
      owner,
      fundsWallet,
      synthetix.address,
      synth.address,
    ...
    expect(await instance.synthetix()).to.equal(synthetix.address);
    expect(await instance.synth()).to.equal(synth.address);
    expect(await instance.fundsWallet()).to.equal(fundsWallet);
    expect(await instance.oracle()).to.equal(oracle);
    expect(await instance.usdToSnxPrice()).to.equal(usdSnx);
    expect(await instance.usdToEthPrice()).to.equal(usdEth);
  });

  //   it("should set funds wallet when invoked by owner", async () => {
  //     const transaction = await depot.setFundsWallet(address1, { from: owner });
  //     assert.eventEqual(transaction, "FundsWalletUpdated", {
  //       newFundsWallet: address1,
  //     });
  ...
  //     await assert.revert(
  //       depot.setFundsWallet(address2, { from: deployerAccount })
  //     );
  //   });
  ...
  //     assert.eventEqual(transaction, "SynthUpdated", {
  //       newSynthContract: address3,
  //     });
  ...
  //     await assert.revert(
  //       depot.setPriceStalePeriod(60, { from: deployerAccount })
  //     );
  //   });
  ...
  //     assert.eventEqual(txn, "PriceStalePeriodUpdated", {
  //       priceStalePeriod: stalePeriod,
  //     });
  ...
  //     const minimumDepositAmount = toUnit("100");
  //     await assert.revert(
  //       depot.setMinimumDepositAmount(minimumDepositAmount, { from: address1 })
  //     );
  //   });
  ...
  //       await synth.methods["transfer(address,uint256)"](
  //         depot.address,
  //         synthsToDeposit,
  //         {
  //           from: depositor,
  ...
  //       const amountDepotReceived = await feePool.amountReceivedFromTransfer(
  //         synthsToDeposit
  //       );
  ...
  //       const synthDepositEvent = events.find(
  //         (log) => log.event === "SynthDeposit"
  //       );
  ...
  //       assert.bnEqual(
  //         await getEthBalance(fundsWallet),
  //         fundsWalletEthBalanceBefore
  //       );
  //     });
  ...
  //       assert.bnEqual(
  //         await getEthBalance(fundsWallet),
  //         fundsWalletEthBalanceBefore.toString()
  //       );
  //     });
  //   });
  ...
  //       const depositTxn = await synth.methods[
  //         "transferSenderPaysFee(address,uint256)"
  //       ](depot.address, synthsToDeposit, {
  //         from: depositor,
  //       });
  ...
  //       const amountReceived = await feePool.amountReceivedFromTransfer(
  //         synthsToDeposit
  //       );
  ...
  //       // Exchange("ETH", msg.value, "sUSD", fulfilled);
  //       const exchangeEvent = transaction.logs.find(
  //         (log) => log.event === "Exchange"
  //       );
  ...
  //       const purchaserSynthBalance = await synth.balanceOf(purchaser);
  //       const depotSynthBalance = await synth.balanceOf(depot.address);
  //       const remainingSynths = web3.utils
  //         .toBN(totalSynthsDeposit)
  //         .sub(synthsAmount);
  //       assert.bnEqual(purchaserSynthBalance, amountReceived);
  ...
  //       const smallDepositsBalance = await depot.smallDeposits(depositor);
  //       assert.bnEqual(
  //         smallDepositsBalance,
  //         synthsToDeposit1.add(synthsToDeposit2)
  //       );
  ...
  //         web3.utils.fromWei(deposit3) -
  //         web3.utils.fromWei(ethToSend) * web3.utils.fromWei(usdEth);
  //       assert.equal(await depot.depositStartIndex(), 2);
  //       assert.equal(await depot.depositEndIndex(), 3);
  //       const totalSellableDeposits = await depot.totalSellableDeposits();
  //       assert.equal(
  //         totalSellableDeposits.toString(),
  //         web3.utils.toWei(remainingSynths.toString())
  //       );
  //     });
  ...
  //       const purchaseValueInSynthsAfterFees =
  //         await feePool.amountReceivedFromTransfer(purchaseValueInSynths);
  //       const purchaseValueInSynthetix = divideDecimal(
  //         purchaseValueInSynthsAfterFees,
  //         snxUSD
  //       );
  ...
  //       const purchaseValueInSynthsAfterFees =
  //         await feePool.amountReceivedFromTransfer(synthsToSend);
  //       const purchaseValueInSynthetix = divideDecimal(
  //         purchaseValueInSynthsAfterFees,
  //         snxUSD
  //       );
