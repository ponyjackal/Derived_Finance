...
contract DebtCache is BaseDebtCache {
    ...
    function takeDebtSnapshot() external override requireSystemActiveIfNotOwner {
        ...
        uint DVDXCollateralDebt;
        for (uint i; i < numValues; i++) {
            uint value = values[i];
            DVDXCollateralDebt = DVDXCollateralDebt.add(value);
            _cachedSynthDebt[currencyKeys[i]] = value;
        }
        _cachedSynthDebt[EXCLUDED_DEBT_KEY] = excludedDebt;
        _cachedDebt = DVDXCollateralDebt.floorsub(excludedDebt);
        _cacheTimestamp = block.timestamp;
        emit DebtCacheUpdated(DVDXCollateralDebt);
    ...
    function _updateCachedSynthDebtsWithRates(
        ...
        if (recomputeExcludedDebt) {
            (uint excludedDebt, bool anyNonDVDXDebtRateIsInvalid) = _totalNonDVDXBackedDebt();
            anyRateIsInvalid = anyRateIsInvalid || anyNonDVDXDebtRateIsInvalid;
