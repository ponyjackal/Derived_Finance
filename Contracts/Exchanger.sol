...
// https://docs.derived.fi/contracts/source/contracts/exchanger
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function maxSecsLeftInWaitingPeriod(address account, bytes32 currencyKey) public override view returns (uint) {
    ...
    function hasWaitingPeriodOrSettlementOwing(address account, bytes32 currencyKey) external override view returns (bool) {
    ...
    function calculateAmountAfterSettlement(
        ...
        uint amount,
        uint refunded
    ) public override view returns (uint amountAfterSettlement) {
    ...
    function isSynthRateInvalid(bytes32 currencyKey) external override view returns (bool) {
    ...
    ) external override onlyDerivedorSynth returns (uint amountReceived) {
        uint fee;
    ...
    function _emitTrackingEvent(
        ...
        IDerivedInternal(address(Derived())).emitExchangeTracking(trackingCode, toCurrencyKey, toAmount);
    }

    function _updateDVDXIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        bool includesUSD = currencyKeys[0] == USDx || currencyKeys[1] == USDx;
        uint numKeys = includesUSD ? 2 : 3;
        ...
        )
    {
        ...
        // in these currencies
        _updateDVDXIssuedDebtOnExchange([sourceCurrencyKey, destinationCurrencyKey], [sourceRate, destinationRate]);
    ...
    function reclaim(
        ...
        IDerivedInternal(address(Derived())).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function refund(
        ...
        IDerivedInternal(address(Derived())).emitExchangeRebate(from, currencyKey, amount);
    }
        ...
        returns (uint exchangeFeeRate)
    {
        ...
        if (sourceCurrencyKey == USDx || destinationCurrencyKey == USDx) {
            return exchangeFeeRate;
        }

        // Is this a swing trade? long to short or short to long skipping USDx.
        if (
    ...
    // Derived or synth check - done this function as part of space constraints of contract while deploying
    function _onlyDerivedorSynth() private view {
    ...
    function _onlyExchangeRates() private view {
