...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    bytes32 internal constant deETH = "deETH";
    ...
    bytes32 private constant CONTRACT_ETHERCOLLATERAL_USDx = "EtherCollateralUSDx";
    ...
    function resolverAddressesRequired() public override view returns (bytes32[] memory addresses) {
        ...
        newAddresses[5] = CONTRACT_ETHERCOLLATERAL_USDx;
    ...
    function etherCollateralUSDx() internal view returns (IEtherCollateralUSDx) {
        return IEtherCollateralUSDx(requireAndGetAddress(CONTRACT_ETHERCOLLATERAL_USDx));
    }
    ...
    function cachedDebt() external override view returns (uint) {
        return _cachedDebt;
    }

    function cachedSynthDebt(bytes32 currencyKey) external override view returns (uint) {
        return _cachedSynthDebt[currencyKey];
    }

    function cacheTimestamp() external override view returns (uint) {
        return _cacheTimestamp;
    }

    function cacheInvalid() external override view returns (bool) {
    ...
    function cacheStale() external override view returns (bool) {
        ...
        )
    {
        ...
        (uint excludedDebt, bool isAnyNonDVDXDebtRateInvalid) = _totalNonDVDXBackedDebt();
        return (values, excludedDebt, isInvalid || isAnyNonDVDXDebtRateInvalid);
    }
    ...
    function cachedSynthDebts(bytes32[] calldata currencyKeys) external override view returns (uint[] memory DVDXIssuedDebts) {
        return _cachedSynthDebts(currencyKeys);
    }

    // Returns the total USDx debt backed by non-DVDX collateral.
    function totalNonDVDXBackedDebt() external override view returns (uint excludedDebt, bool isInvalid) {
        return _totalNonDVDXBackedDebt();
    }

    function _totalNonDVDXBackedDebt() internal view returns (uint excludedDebt, bool isInvalid) {
        // Calculate excluded debt.
        // 1. Ether Collateral.
        excludedDebt = excludedDebt.add(etherCollateralUSDx().totalIssuedSynths()); // Ether-backed USDx

        uint etherCollateralTotalIssuedSynths = etherCollateral().totalIssuedSynths();
        // We check the supply > 0 as on L2, we may not yet have up-to-date rates for deETH.
        if (etherCollateralTotalIssuedSynths > 0) {
            (uint deETHRate, bool deETHRateIsInvalid) = exchangeRates().rateAndInvalid(deETH);
            isInvalid = isInvalid || deETHRateIsInvalid;
            excludedDebt = excludedDebt.add(etherCollateralTotalIssuedSynths.multiplyDecimalRound(deETHRate)); // Ether-backed deETH
        }
        ...
        // 3. EtherWrapper.
        // Subtract deETH and USDx issued by EtherWrapper.
    ...
    function _currentDebt() internal view returns (uint debt, bool anyRateIsInvalid) {
        ...
        return (total, isInvalid || isAnyNonDVDXDebtRateInvalid);
    }

    function currentDebt() external override view returns (uint debt, bool anyRateIsInvalid) {
        return _currentDebt();
    }
    ...
    function updateCachedSynthDebts(bytes32[] calldata currencyKeys) external virtual override {}

    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint currencyRate) external virtual override {}

    function updateCachedSynthDebtsWithRates(bytes32[] calldata currencyKeys, uint[] calldata currencyRates) external virtual override {}

    function updateDebtCacheValidity(bool currentlyInvalid) external virtual override {}

    function purgeCachedSynthDebt(bytes32 currencyKey) external virtual override {}

    function takeDebtSnapshot() external virtual override {}
