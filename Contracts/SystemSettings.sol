...
// https://docs.derived.fi/contracts/source/contracts/systemsettings
contract SystemSettings is Owned, MixinSystemSettings, ISystemSettings {
    ...
    uint constant MAX_ISSUANCE_RATIO = 1e18;

    // The fee period must be between 1 day and 60 days.
    uint constant MIN_FEE_PERIOD_DURATION = 1 days;
    uint constant MAX_FEE_PERIOD_DURATION = 60 days;

    uint constant MAX_TARGET_THRESHOLD = 50;

    uint constant MAX_LIQUIDATION_RATIO = 1e18; // 100% issuance ratio

    uint constant MAX_LIQUIDATION_PENALTY = 1e18 / 4; // Max 25% liquidation penalty / bonus

    uint constant RATIO_FROM_TARGET_BUFFER = 2e18; // 200% - mininimum buffer between issuance ratio and liquidation ratio

    uint constant MAX_LIQUIDATION_DELAY = 30 days;
    uint constant MIN_LIQUIDATION_DELAY = 1 days;

    // Exchange fee may not exceed 10%.
    uint constant MAX_EXCHANGE_FEE_RATE = 1e18 / 10;

    // Minimum Stake time may not exceed 1 weeks.
    uint constant MAX_MINIMUM_STAKE_TIME = 1 weeks;

    uint constant MAX_CROSS_DOMAIN_GAS_LIMIT = 8e6;
    uint constant MIN_CROSS_DOMAIN_GAS_LIMIT = 3e6;

    // these are simple bounds for the mint/burn fee rates (max 100%).
    // Can we come up with better values?
    uint constant MAX_ETHER_WRAPPER_MINT_FEE_RATE = 1e18;
    uint constant MAX_ETHER_WRAPPER_BURN_FEE_RATE = 1e18;
    ...
    // before settlement.
    function waitingPeriodSecs() external override view returns (uint) {
    ...
    function priceDeviationThresholdFactor() external override view returns (uint) {
        return getPriceDeviationThresholdFactor();
    }

    // The raio of collateral
    // Expressed in 18 decimals. So 800% cratio is 100/800 = 0.125 (0.125e18)
    function issuanceRatio() external override view returns (uint) {
    ...
    function liquidationDelay() external override view returns (uint) {
    ...
    function liquidationRatio() external override view returns (uint) {
        return getLiquidationRatio();
    }

    // SIP-15 Liquidations
    // penalty taken away from target of liquidation (with 18 decimals). E.g. 10% is 0.1e18
    function liquidationPenalty() external override view returns (uint) {
        return getLiquidationPenalty();
    }

    // How long will the ExchangeRates contract assume the rate of any asset is correct
    function rateStalePeriod() external override view returns (uint) {
        return getRateStalePeriod();
    }

    function exchangeFeeRate(bytes32 currencyKey) external override view returns (uint) {
        return getExchangeFeeRate(currencyKey);
    }

    function minimumStakeTime() external override view returns (uint) {
    ...
    function etherWrapperMaxETH() external override view returns (uint) {
        return getEtherWrapperMaxETH();
    }

    // SIP 112: ETH Wrappr
    // The fee for depositing ETH into the EtherWrapper.
    function etherWrapperMintFeeRate() external override view returns (uint) {
        return getEtherWrapperMintFeeRate();
    }

    // SIP 112: ETH Wrappr
    // The fee for burning sETH and releasing ETH from the EtherWrapper.
    function etherWrapperBurnFeeRate() external override view returns (uint) {
    ...
    function setIssuanceRatio(uint _issuanceRatio) external onlyOwner {
        require(_issuanceRatio <= MAX_ISSUANCE_RATIO, "2");
    ...
    function setFeePeriodDuration(uint _feePeriodDuration) external onlyOwner {
        require(_feePeriodDuration >= MIN_FEE_PERIOD_DURATION, "3");
        require(_feePeriodDuration <= MAX_FEE_PERIOD_DURATION, "4");
    ...
    function setTargetThreshold(uint _percent) external onlyOwner {
        require(_percent <= MAX_TARGET_THRESHOLD, "5");
    ...
    function setLiquidationDelay(uint time) external onlyOwner {
        require(time <= MAX_LIQUIDATION_DELAY, "6");
        require(time >= MIN_LIQUIDATION_DELAY, "7");
    ...
    function setLiquidationRatio(uint _liquidationRatio) external onlyOwner {
        ...
        require(_liquidationRatio >= MIN_LIQUIDATION_RATIO, "9");
    ...
    function setLiquidationPenalty(uint penalty) external onlyOwner {
        require(penalty <= MAX_LIQUIDATION_PENALTY, "10");
    ...
    function setExchangeFeeRateForSynths(bytes32[] calldata synthKeys, uint256[] calldata exchangeFeeRates)
        ...
        onlyOwner
    {
        require(synthKeys.length == exchangeFeeRates.length, "11");
        for (uint i = 0; i < synthKeys.length; i++) {
            require(exchangeFeeRates[i] <= MAX_EXCHANGE_FEE_RATE, "12");
            flexibleStorage().setUIntValue(
                SETTING_CONTRACT_NAME,
    ...
    function setMinimumStakeTime(uint _seconds) external onlyOwner {
        require(_seconds <= MAX_MINIMUM_STAKE_TIME, "13");
    ...
    function setEtherWrapperMintFeeRate(uint _rate) external onlyOwner {
        require(_rate <= MAX_ETHER_WRAPPER_MINT_FEE_RATE, "15");
    ...
    function setEtherWrapperBurnFeeRate(uint _rate) external onlyOwner {
        require(_rate <= MAX_ETHER_WRAPPER_BURN_FEE_RATE, "16");
