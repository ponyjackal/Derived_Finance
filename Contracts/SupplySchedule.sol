...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    // The number of DVDX rewarded to the caller of Derived.mint()
    uint public override minterReward = 200 * SafeDecimalMath.unit();
    ...
    // Address of the DerivedProxy for the onlyDerived modifier
    address payable public DerivedProxy;
     ...
     */
    function mintableSupply() external override view returns (uint) {
        uint totalAmount;
        ...
        while (remainingWeeksToMint > 0) {
            ...
            if (currentWeek < SUPPLY_DECAY_START) {
                ...
                // Terminal supply is calculated on the total supply of Derived including any new supply
                // We can compound the remaining week's supply at the fixed terminal rate
                uint totalSupply = IERC20(DerivedProxy).totalSupply();
    ...
    /**
     * @notice Record the mint event from Derived by incrementing the inflation
     * week counter for the number of weeks minted (probabaly always 1)
     * and store the time of the event.
     * @param supplyMinted the amount of DVDX the total supply was inflated by.
     * */
    function recordMintEvent(uint supplyMinted) external override onlyDerived returns (bool) {
    ...
    /**
     * @notice Set the DerivedProxy should it ever change.
     * SupplySchedule requires Derived address as it has the authority
     * to record mint event.
     * */
    function setDerivedProxy(IDerived _DerivedProxy) external onlyOwner {
        require(address(_DerivedProxy) != address(0), "Address cannot be 0");
        DerivedProxy = payable(address (uint160(address(_DerivedProxy))));
        emit DerivedProxyUpdated(DerivedProxy);
    }

    // ========== MODIFIERS ==========

    /**
     * @notice Only the Derived contract is authorised to call this function
     * */
    modifier onlyDerived() {
        require(
            msg.sender == address(Proxy(payable(DerivedProxy)).target()),
            "Only the Derived contract can perform this action"
        );
        _;
    }
    ...
    /**
     * @notice Emitted when setDerivedProxy is called changing the Derived Proxy address
     * */
    event DerivedProxyUpdated(address newAddress);
}
