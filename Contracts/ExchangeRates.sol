...
contract ExchangeRates is Owned, MixinSystemSettings, IExchangeRates {
    ...
    mapping(bytes32 => AggregatorV2V3Interface) new_aggregators;
    ...
    {
        RateAndUpdatedTime memory rateAndTime =
            _getRateAndUpdatedTime(currencyKey);
    ...
    function effectiveValueAtRound(
        ...
        (uint256 srcRate, ) =
            _getRateAndTimestampAtRound(sourceCurrencyKey, roundIdForSrc);
        (uint256 destRate, ) =
            _getRateAndTimestampAtRound(destinationCurrencyKey, roundIdForDest);
        if (destRate == 0) {
        ...
        returns (uint256[] memory rates, bool anyRateInvalid)
    {
        ...
        for (uint256 i = 0; i < currencyKeys.length; i++) {
            ...
            RateAndUpdatedTime memory rateEntry =
                _getRateAndUpdatedTime(currencyKeys[i]);
    ...
    function getTokenPrice(uint256 amount) public view returns (uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(dvdxPair);
        ...
        returns (bool[] memory flagList)
    {
        ...
        if (_flags != FlagsInterface(address(0))) {
            ...
            for (uint256 i = 0; i < currencyKeys.length; i++) {
                _aggregators[i] = address(new_aggregators[currencyKeys[i]]);
            }
        ...
        returns (uint256)
    {
        ...
        if (currencyKeyDecimals[currencyKey] > 0) {
            uint256 multiplier =
                10**uint256(SafeMath.sub(18, currencyKeyDecimals[currencyKey]));
    ...
    {
        AggregatorV2V3Interface aggregator = new_aggregators[currencyKey];
        ...
        if (aggregator != AggregatorV2V3Interface(address(0))) {
            ...
            (bool success, bytes memory returnData) =
                address(aggregator).staticcall(payload);
            ...
            uint256 roundId = currentRoundForRate[currencyKey];
            RateAndUpdatedTime memory entry = _rates[currencyKey][roundId];

            return
                RateAndUpdatedTime({
                    rate: uint216(
                        _rateOrInverted(currencyKey, entry.rate, roundId)
                    ),
                    time: entry.time
                });
        }
        ...
        returns (uint256 rate, uint256 time)
    {
        ...
        if (aggregator != AggregatorV2V3Interface(address(0))) {
            ...
            return (
                _rateOrInverted(currencyKey, update.rate, roundId),
                update.time
            );
        }
    }
