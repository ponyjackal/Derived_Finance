...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function resolverAddressesRequired() public override view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_SYNTHUSDx;
    ...
    function _synthUSDx() internal view returns (ISynth) {
    ...
    function openInternal(
        ...
        // 5. Check we haven't hit the debt cap for non DVDX collateral.
        ...
        // 14. If its short, convert back to USDx, otherwise issue the loan.
        if (short) {
            _synthUSDx().issue(msg.sender, _exchangeRates().effectiveValue(currency, loanAmountMinusFee, USDx));
    ...
    function drawInternal(uint id, uint amount) internal rateIsValid {
        ...
        if (loan.short) {
            ...
            _synthUSDx().issue(msg.sender, _exchangeRates().effectiveValue(loan.currency, amountMinusFee, USDx));
    ...
    // Take an amount of fees in a certain synth and convert it to USDx before paying the fee pool.
    function _payFees(uint amount, bytes32 synth) internal {
        if (amount > 0) {
            if (synth != USDx) {
                amount = _exchangeRates().effectiveValue(synth, amount, USDx);
            }
            _synthUSDx().issue(_feePool().FEE_ADDRESS(), amount);
